...
library(rlang)
library(dyngen)
# optional, if don't have manuscript installed, use cell below
...
design_datasets <- exp$result("design_datasets.rds") %cache% {
  crossing(
    ...
    backbone_name = names(list_backbones())[c(1, 5, 9, 12, 14)], 
    ...
    # number of cells
    n_cells = c(100, 150, 200, 250, 300)
  ) %>%
    mutate(id = paste0(backbone_name, "_", tr_rate_multiplier , "_", spl_rate_multiplier, "_", degrade_multiplier, '_', n_cells))
}
design_datasets
```
...
## run simulator
```{r}
pwalk(design_datasets, function(id, seed, backbone_name, tr_rate_multiplier, spl_rate_multiplier, degrade_multiplier, n_cells) {
  if (!file.exists(exp$dataset_file(id))) {
    ...
    # num_cell params, 2000 cells
    num_cells <- n_cells # 4000 #2000
    ...
    simulation_params <- simulation_default(
      census_interval = 1,
      experiment_params = bind_rows(simulation_type_wild_type(num_simulations = 3),
...
```{r}
pwalk(
  design_datasets, function(id, seed, backbone_name, tr_rate_multiplier, spl_rate_multiplier, degrade_multiplier, n_cells){
    ...
    # write.csv(spliced_t,file = paste0("./mixed_data/",id,"_spliced_t.csv"))
    # write.csv(unspliced_t,file = paste0("./mixed_data/",id,"_unspliced_t.csv"))

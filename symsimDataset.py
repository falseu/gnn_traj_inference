...
class SymsimCycle(InMemoryDataset):
    ...
    def process(self):
        ...
        for file_name in tree_files:
            ...
            data = Data(x=x, edge_index=edge_index, y=y, adj=adj, v=v, y_dpt = y_dpt)
            data_list.append(data)
        data, slices = self.collate(data_list)
        torch.save((data, slices), self.processed_paths[0])       
    def __repr__(self):
...
class SymsimBranchNoisy(InMemoryDataset):

    def __init__(self, root='data/', transform=None, pre_transform=None):
        super(SymsimBranchNoisy, self).__init__(root, transform, pre_transform)
        self.data, self.slices = torch.load(self.processed_paths[0])
    ...
    @property
    def processed_file_names(self):
        return ['SymsimBranchNoisy.dataset']
...
class SymsimCycleNoisy(InMemoryDataset):

    def __init__(self, root='data/', transform=None, pre_transform=None):
        super(SymsimCycleNoisy, self).__init__(root, transform, pre_transform)
        self.data, self.slices = torch.load(self.processed_paths[0])
    ...
    @property
    def processed_file_names(self):
        return ['SymsimCycleNoisy.dataset']

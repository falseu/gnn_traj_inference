...
design_datasets <- exp$result("design_datasets.rds") %cache% {
  crossing(
    ...
    spl_rate_multiplier = c(10,1,0.1),
    degrade_multiplier = c(1),
    num_cells = c(1000, 1500, 2000),
  ) %>%
    mutate(id = paste0(backbone_name, "_", tr_rate_multiplier , "_", spl_rate_multiplier, "_", num_cells))
}
design_datasets
```
...
pwalk(
  design_datasets, function(id, seed, backbone_name, tr_rate_multiplier, spl_rate_multiplier, degrade_multiplier, num_cells){
    ...
    write.csv(spliced,file = paste0("./result_files/",id,"_spliced.csv"))
    write.csv(unspliced,file = paste0("./result_files/",id,"_unspliced.csv"))
    write.csv(protein,file = paste0("./result_files/",id,"_unspliced.csv"))
    write.csv(cell_info,file = paste0("./result_files/",id,"_cell_info.csv"))

    write.csv(spliced_t,file = paste0("./result_files/",id,"_spliced_t.csv"))

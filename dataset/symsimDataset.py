...
class SymsimNewNoVeloBranch(InMemoryDataset):

    def __init__(self, root='data/', transform=None, pre_transform=None):
        super(SymsimNewNoVeloBranch, self).__init__(root, transform, pre_transform)
        self.data, self.slices = torch.load(self.processed_paths[0])
    ...
    @property
    def processed_file_names(self):
        return ['SymsimNewNoVeloBranch.dataset']
    ...
    def process(self):
        ...
        for file_name in tree_files:
            X = pd.read_csv(root + file_name + "counts.txt", sep="\t",header=None).to_numpy().T
            true_time = pd.read_csv(root + file_name + "cell_labels.txt", sep = "\t",lineterminator="\n", header = 0).to_numpy()
            X_obs = pd.DataFrame(data=true_time, index = ['cell_' + str(x) for x in range(X.shape[0])], columns = ['back_bone','sim_time'])
            adata = anndata.AnnData(X = csr_matrix(X), obs = X_obs)

            idx = [adata.obs['back_bone'] == branch for branch in branchs]
